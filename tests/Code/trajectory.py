...
def model():
    ...
    temperature_curve = np.zeros(500)

    for i in range(500):
        temperature_curve[i] = 250
    ...
    plt.plot(t_range, temperature, label=f'Temperatura Simulada',color = 'red')
    plt.plot(t_range, app_pot, label=f'Potencia Aplicada',color = 'yellow')

    plt.xlabel('Time', fontsize=12)
    plt.ylabel('Temperature', fontsize=12)
    plt.title('Simulação Simplificada Do Modelo', fontsize=14)
