...
def cubic_interpolation_tangent(csv_file, dcont = 7000, dini = 1000):
    ...
    tau_dead = time[next(i for i, temp in enumerate(temperature) if temp >= threshold_value)] - time[0]
    ...
    tau_planta = (((sept_interp(real_roots[0]) - tangent_point))/tangent_slope + time_interp[index]) - tau_dead
    
    print("\n")
    print("Open-Loop Data")
    print(f"Delta_controle: {dcont} units")
    print(f"Delta_planta: {temp_diff} ºC")
    print(f"Tau_dead: {tau_dead} seconds")
    print(f"Tau_planta: {tau_planta} seconds")
    print("\n")

    Rplant = tau_dead/ tau_planta
    bK_0 = (dcont/temp_diff)*Rplant
    print(f"R: {Rplant}")
    print(f"K0: {bK_0}")
    print("\n")
    ...
    ccP = dcont/dini
    ccN = (abs(temp_diff)/temperature[0])/tau_planta
    ccL = tau_dead
    ccR = Rplant

    ccK_p = (ccP/(ccN *ccL)) * (1.33 + (ccR/4))
    ccK_i = ccK_p/((ccL*(30+3*ccR))/(9 + 20*ccR))
    ccK_d = ccK_p*(4*ccL/(11 + 2*ccR))
...
if __name__ == "__main__":
    ...
    csv_file_path = 'Data/botafogo.csv'
    cubic_interpolation_tangent(csv_file_path,7000,1000)